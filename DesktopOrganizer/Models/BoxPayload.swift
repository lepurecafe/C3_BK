...
// volumetric 박스 창을 열 때 WindowGroup에 전달하는 값입니다.
//
// SwiftUI의 WindowGroup(id:for:)는 Hashable/Codable 값을 받아 새 창을 식별하고 복원합니다.
// 그래서 SwiftData 모델인 OrganizerBox를 직접 넘기지 않고, 창 생성에 필요한 최소 값만 담은 payload를 사용합니다.
struct BoxPayload: Hashable, Codable, Identifiable {
    ...
    // 현재 volumetric window API는 openWindow payload의 위치를 직접 배치에 쓰지는 않지만,
    // 이후 WorldAnchor/공간 배치로 확장할 때 이어 받을 좌표입니다.
    var posX: Float = 0
    var posY: Float = -0.3
    var posZ: Float = -0.8
